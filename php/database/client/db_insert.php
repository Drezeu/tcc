<?php
	...
	if(strcasecmp($_POST['form_action'], 'new_client') == 0){
		...
		if(isset($_POST['user'])){
			...
			$sql = "INSERT INTO TB_CLIENTS VALUES (NULL, '".$_POST['user']."', '".$_POST['email']."', '".$password_hash."', 0, '".$_POST['complete_name']."', '".$_POST['birthdate']."', '".$_POST['cpf']."', '".$_POST['phone']."', (SELECT CD_ADDRESS FROM TB_ADDRESS ORDER BY CD_ADDRESS DESC LIMIT 1), (SELECT CD_CLIENT_IMG FROM TB_CLIENTS_IMGS ORDER BY CD_CLIENT_IMG DESC LIMIT 1))";
			if($query = $mysqli->query($sql)){}
			else{
